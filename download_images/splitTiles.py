...
if __name__ == "__main__":
    ...
    for i in range(len(images)):
        ...
        red_image_tiels = os.listdir(OUTPUT + '/' + images[i] + '/red')
        if not os.path.exists(OUTPUT + '/' + images[i] + '/trueColor'):
            os.mkdir(OUTPUT + '/' + images[i] + '/trueColor')
        for j in range(len(red_image_tiels)):
            print(f'({j + 1}/{len(red_image_tiels)})')
            os.system(f'gdal_merge.py -separate -o {OUTPUT + '/' + images[i] + '/trueColor/' + red_image_tiels[j]} {OUTPUT + '/' + images[i] + '/red/' + red_image_tiels[j]} {OUTPUT + '/' + images[i] + '/green/' + red_image_tiels[j]} {OUTPUT + '/' + images[i] + '/blue/' + red_image_tiels[j]}')
